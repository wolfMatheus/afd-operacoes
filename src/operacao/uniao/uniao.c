...
AFD *uniao(AFD *afd1, AFD *afd2)
{
  ...
  int numeroEstadoFinal = ((*afd1->numero_estado_final) * (*afd2->estados_numericos)) +
                          ((*afd2->numero_estado_final) * (*afd1->estados_numericos)) - (*afd1->numero_estado_final * (*afd2->numero_estado_final));
  ...
  *produto->numero_estado_final = numeroEstadoFinal;
  ...
  for (int i = 0; i < *afd1->numero_estado_final; i++)
  {
    ...
    int startingPoint = estadoFinal * (*afd2->estados_numericos);
  ...
  for (int i = 0; i < *afd2->numero_estado_final; i++)
  {
    ...
    for (int j = 0; j < *afd1->estados_numericos; j++)
    {
      int indice = (j * (*afd2->estados_numericos)) + estadoFinal;
